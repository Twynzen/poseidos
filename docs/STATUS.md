...
- **Última rutina:** 2026-08-13 — dry-fire click (X sin munición)
- **Qué quedó (esta corrida):**
  - `combatPlayer.ts`: `CombatSfxKind` +`"dry"`; `combatBeepSpec("dry")` 1100Hz square 35ms, gain 0.06. `playDryFire` via `playKind`. Mute → no-op. AudioContext lazy.
  - `game.ts`: `consumeShoot` fail + `shot.message === "sin munición"` → `playDryFire`. `"sin pistola"` HUD-only. Fail sin tracer/flash/spark/noise.
