...
- **Última rutina:** 2026-08-13 — HUD cerca omite contenedores vacíos
- **Qué quedó (esta corrida):**
  - `containerHasLoot`: `slots.length>0 && totalQty>0`.
  - `nearest()` salta `!containerHasLoot`.
  - game: `nearHint` solo si `near && containerHasLoot(near)`; `emptyLootIds` usa el mismo helper.
  - Tests: inventory + hudStatus.
