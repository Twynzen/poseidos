...
- **Última rutina:** 2026-08-13 — loot floater DOM toast (inline styles, z 9999)
- **Qué quedó (esta corrida):**
  - visibilidad: live `getElementById` o crea `#loot-floater` al final de body; inline `position:fixed; z-index:9999; font 36px; color:#ffe080`; inyecta `@keyframes loot-float` 1.8s.
  - tests: bag `querySelector("#loot-floater")` / classList reflow (sin jsdom).
  - `showLootFloaterHud` tras loot G/E (`spawnLootFloater` + toast).
