...
- **Última rutina:** 2026-08-13 — loot nameplates flotantes
- **Qué quedó (esta corrida):**
  - `lootNameplate.ts`: truncate label 16; fade dist 10 (opacity 1@0 → 0@10); y 1.45; visible = opacity>0 y no empty.
  - `worldView`: Sprite canvas `lootNameplate` hijo de cada grupo loot (nombre del contenedor); `syncLootFocus` aplica visible/opacity. Dispose de map/material.
