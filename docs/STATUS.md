...
- **Última rutina:** 2026-08-13 — HUD pill DIA/NOC
- **Qué quedó (esta corrida):**
  - `moodles.ts`: `MoodleId` +`"clock"`; `CLOCK_NIGHT_DARK=30`; `clockMoodle(isNight, phasePct)` — día **DIA**/☀ ok; noche **NOC**/☾ warn si pct<30, si no critical. Valor `Math.round(phasePct)`. `buildHudMoodles` concatena clock al final. `buildMoodles` sigue en 4 pills.
  - `game.ts`: ambos `moodlesHud.sync` vía `buildPlayerHudMoodles` pasan `clock.isNight` y `phase*100`.
