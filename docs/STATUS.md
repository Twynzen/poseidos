...
- **Última rutina:** 2026-08-13 — loot focus pulse
- **Qué quedó (esta corrida):**
  - `lootFocus`: scale 1.35@0 → 1.12@reach 1.6 (1.0 fuera); pulse `1+0.08*sin(t*6)`; mul = scale*pulse in reach.
  - `worldView.syncLootFocus(wx,wy,dt)`: el contenedor más cercano en reach recibe el mul; el resto scale 1.
  - `game`: sync cada frame (vivo + game-over) tras `syncPlayer`.
