...
- **Última rutina:** 2026-08-12 — attack/hit one-shot wiring
- **Qué quedó (esta corrida):**
  - `worldView.triggerPlayerAction("primary-attack"|"hit")`: `setAction` en playerAnimator + mixer sync.
  - `characterMixer`: clip ausente = no-op (Soldier); one-shots `reset()` si el clip existe (Survivor drop-in).
  - `game.ts`: `tryMelee` / `tryShoot` ok → primary-attack; toque hostil `takeDamage` → hit (no needs DPS).
...
- **Siguiente subtarea concreta:** export Mesh2Motion real a `Survivor.glb` / death clip
- **Bloqueos:** ninguno
