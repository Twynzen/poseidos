...
- **Última rutina:** 2026-08-13 — ocultar anillos de loot vacíos
- **Qué quedó (esta corrida):**
  - `lootRingVisible(empty)` → `!empty`.
  - worldView: grupos `{group,x,y,id}`; `syncLootFocus(..., emptyIds)` oculta y no foca vacíos.
  - game: `emptyLootIds` de `containers.list` (`slots.length===0` o `totalQty===0`).
  - Tests: `lootRingVisible`.
