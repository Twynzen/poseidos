...
- **Última rutina:** 2026-08-12 — walk facing continuo + survivor look tint
- **Qué quedó (esta corrida):**
  - `playerFacing.ts`: `PLAYER_GLTF_YAW_OFFSET=0` + `playerGltfYawFromMove` (atan2; null si ~0).
  - `game.ts` pasa ejes vivos (`axes.x/z`) a `tickPlayerLoco` al mover; facing cardinal intacto para melee/barricada/disparo.
  - `worldView.tickPlayerLoco` usa `playerGltfYawFromMove` (diagonales continúas, no snap N/E/S/W).
  - `survivorLook.ts`: `applySurvivorLook` — paleta tierra/gris, clona mats, acento visor/helmet; cableado en `onAttached` tras `bindMixer` OK.
...
- **Siguiente subtarea concreta:** Mesh2Motion survival GLB (reemplazar Soldier + tint)
- **Bloqueos:** ninguno
