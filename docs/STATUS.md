...
- **Última rutina:** 2026-08-13 — cono de linterna al facing
- **Qué quedó (esta corrida):**
  - `flashlightCone.ts`: `FLASHLIGHT_CONE_LENGTH=4.2` / `HALF_WIDTH=1.15` / yaw offset 0; tip (sin, cos) XZ, yaw 0 → +Z. Wedge ápice-left-right. `flashlightConeAngle` = atan(hw/L). No re-aplica `PLAYER_GLTF_YAW_OFFSET`.
  - `worldView.ts`: SpotLight `0xb0d0ff` + wedge unlit `flashlightConeWedge` (MeshBasic, additive, y=0.05). PointLight fill se mantiene. `syncTorchLight` apunta con `playerGltfYaw`.
