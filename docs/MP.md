...
| Possession trust / gates | sí | trust + TTLs + lastApplied / lastRejected / gateLine / moodBias / toneBias / memorySummary / lineSource / line / tone / trigger (ya validados); clientes no ven intents |
...
Wire `possession: NetPossessionSnap[]` — por poseído: `id`, `trust`, `pacifiedLeft`, `speedBumpLeft`, `speedBumpMul` (1 si inactivo), `pacified` (trust OR TTL), `lastApplied?` / `lastRejected?` (`GateTag[]` ya validados; omitidos si vacíos), `gateLine?` (última línea ya validada `gates.gateLine`; omitida si vacía; cap `GATE_LINE_MAX_LEN`; no se reformatea), `moodBias?` (`PossessionTone` ya validado `speech.getMoodBias`; omitido si vacío / null / desconocido; distinto de `ShortMemory.toneBias` y del tono de utterance), `toneBias?` (`PossessionTone` ya validado `memory.toneBias`; omitido si vacío / null / desconocido; distinto de speech `moodBias` y del tono de utterance), `memorySummary?` (resumen compacto ya validado `formatMemorySummary(memory.recent)`; omitido si vacío / null / whitespace; cap `MEMORY_SUMMARY_MAX_LEN`; distinto de `toneBias`), `lineSource?` (`LineSource` ya validado `speech.getActive.lineSource` → `llm` | `bank`; omitido si vacío / null / desconocido / sin utterance; distinto de `memorySummary`; no se mapea a STUB/BANCO), `line?` (última línea hablada ya validada `speech.getActive.line`; omitida si vacía / null / whitespace / sin utterance; cap `LLM_LINE_MAX_LEN` via `compactLlmLine`; distinto de `lineSource`, de `gateLine` y del tono de utterance), `tone?` (`PossessionTone` ya validado `speech.getActive.tone` → `lucidez` | `demonio` | `ruega`; omitido si vacío / null / undefined / desconocido / sin utterance; via `compactMoodBias`; distinto de `moodBias`, de `toneBias` y de `line`), `trigger?` (`SpeechTrigger` ya validado `speech.getActive.trigger` → `periodic` | `see_player` | `dialogue`; omitido si vacío / null / undefined / desconocido / sin utterance; distinto de `tone`, de `line` y de `lineSource`). Collector: `collectPossessionFrom(ledger, gates, ids?, moodBiasOf?, toneBiasOf?, memoryOf?, lineSourceOf?, lineOf?, toneOf?, triggerOf?)`. Host hook: `collectHostPossessionFrom(ledger, gates, ids?, speech, memory)` pasa SpeechDirector + ShortMemory existentes (sin segundo lookup). Path 3-arg sin cambio. Game 1P: cada tick `publishHostPossession` → `LocalLoopbackSession.setPossession` (`trust.ids()`; sin WebSocket). Game 1P también: cada tick `publishHostHostiles` → `LocalLoopbackSession.setHostilesFromSim` (id / x / y / mode; sin WebSocket). Game 1P también: cada tick `publishHostDoors` → `collectDoorsFromMap` → `LocalLoopbackSession.setDoors` (x / y / open; sin WebSocket). Game 1P también: cada tick `publishHostContainers` → `collectContainersFromRegistry` → `LocalLoopbackSession.setContainers` (id / x / y / slots qty>0; sin WebSocket). Game 1P también: cada tick `publishHostBarricades` → `collectBarricadesFromMap` → `LocalLoopbackSession.setBarricades` (x / y; sin WebSocket).
