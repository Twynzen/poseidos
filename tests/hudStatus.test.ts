...
describe("hudAccAfterRestart (R / softReset)", () => {
  ...
  test("Game softReset usa helper tras refreshHud; freeze R no pisa 1; F9/enterGameOver siguen 1", () => {
    ...
    expect(gameSrc).toMatch(
      /softReset\(\): void \{[\s\S]{0,4700}this\.refreshHud\(true\);[\s\S]{0,200}this\.hudAcc = hudAccAfterRestart\(\)/,
    );
    expect(gameSrc).not.toMatch(
...
describe("death → R HUD lock (softReset)", () => {
  ...
  test("Game softReset assign lastLootMsg/gameOver/hudAcc/spawnGrace; refreshHud limpia HAS MUERTO", () => {
    ...
    expect(gameSrc).toMatch(
      /softReset\(\): void \{[\s\S]{0,3700}this\.spawnGrace = spawnGraceAfterRestart\(\)/,
    );
    expect(gameSrc).toMatch(
      /this\.lastLootMsg = lastLootMsgAfterRestart\(\);[\s\S]{0,2600}this\.refreshHud\(true\);[\s\S]{0,200}this\.hudAcc = hudAccAfterRestart\(\)/,
    );
    expect(gameSrc).not.toMatch(
