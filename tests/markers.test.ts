...
describe("markers (badges + ground rings)", () => {
  ...
  test("paleta: bed púrpura sleep (anillo 0x8c73a6, badge 0xc1a6d4, ▭)", () => {
    const bed = paletteFor("bed");
    expect(bed.ring).toBe(0x8c73a6);
    expect(bed.badge).toBe(0xc1a6d4);
    expect(bed.emissive).toBe(0x251c2e);
  ...
  test("player ring 0x3a7fd4 × 1.15/canal → 0x4392f4; glyph/other-roles/opacities iguales", () => {
    ...
    expect(MARKER_PALETTE.bed.ring).toBe(0x8c73a6);
    expect(PLAYER_FOOT_RING_OPACITY).toBe(0);
    expect(MARKER_RING_OPACITY).toBe(0.6877);
  });
  ...
  test("player badge 0x7eb6ef × 1.15/canal (b clamp) → 0x91d1ff; ring/glyph/other-roles/opacities iguales", () => {
    ...
    expect(MARKER_PALETTE.bed.badge).toBe(0xc1a6d4);
  ...
  test("player emissive 0x1a4060 × 1.15/canal → 0x1e4a6e; ring/badge/glyph/other-roles/opacities iguales", () => {
    ...
    expect(MARKER_PALETTE.bed.emissive).toBe(0x251c2e);
    expect(PLAYER_BADGE_OPACITY).toBe(0);
    expect(PLAYER_FOOT_RING_OPACITY).toBe(0);
    expect(MARKER_RING_OPACITY).toBe(0.6877);
  });
  ...
  test("bed ring 0x7a6490 × 1.15/canal → 0x8c73a6; badge/emissive/glyph/other-roles/opacities iguales", () => {
    const bed = paletteFor("bed");
    expect(bed.ring).toBe(0x8c73a6);
    const r = (bed.ring >> 16) & 0xff;
    const g = (bed.ring >> 8) & 0xff;
    const b = bed.ring & 0xff;
    expect(r).toBe(0x8c);
    expect(g).toBe(0x73);
    expect(b).toBe(0xa6);
    expect(Math.round((0x7a * 115) / 100)).toBe(r);
    expect(Math.round((0x64 * 115) / 100)).toBe(g);
    expect(Math.round((0x90 * 115) / 100)).toBe(b);
    expect(bed.badge).toBe(0xc1a6d4);
    expect(bed.emissive).toBe(0x251c2e);
    expect(bed.glyph).toBe("▭");
    expect(MARKER_PALETTE.player.ring).toBe(0x4392f4);
  ...
  test("bed badge 0xa890b8 × 1.15/canal → 0xc1a6d4; ring/emissive/glyph/other-roles/opacities iguales", () => {
    const bed = paletteFor("bed");
    expect(bed.badge).toBe(0xc1a6d4);
    const r = (bed.badge >> 16) & 0xff;
    const g = (bed.badge >> 8) & 0xff;
    const b = bed.badge & 0xff;
    expect(r).toBe(0xc1);
    expect(g).toBe(0xa6);
    expect(b).toBe(0xd4);
    expect(Math.round((0xa8 * 115) / 100)).toBe(r);
    expect(Math.round((0x90 * 115) / 100)).toBe(g);
    expect(Math.round((0xb8 * 115) / 100)).toBe(b);
    expect(bed.ring).toBe(0x8c73a6);
    expect(bed.emissive).toBe(0x251c2e);
    expect(bed.glyph).toBe("▭");
  ...
  test("bed emissive 0x201828 × 1.15/canal → 0x251c2e; ring/badge/glyph/other-roles/opacities iguales", () => {
    const bed = paletteFor("bed");
    expect(bed.emissive).toBe(0x251c2e);
    const r = (bed.emissive >> 16) & 0xff;
    const g = (bed.emissive >> 8) & 0xff;
    const b = bed.emissive & 0xff;
    expect(r).toBe(0x25);
    expect(g).toBe(0x1c);
    expect(b).toBe(0x2e);
    expect(Math.round((0x20 * 115) / 100)).toBe(r);
    expect(Math.round((0x18 * 115) / 100)).toBe(g);
    expect(Math.round((0x28 * 115) / 100)).toBe(b);
    expect(bed.ring).toBe(0x8c73a6);
    expect(bed.badge).toBe(0xc1a6d4);
    expect(bed.glyph).toBe("▭");
  ...
  test("door/bed/loot/mute/possessed floatBadge Y 2.645", () => {
    ...
    expect(muteBadgeY).toBeCloseTo(2.3 * 1.15, 10);
    expect(possessedBadgeY).toBe(2.645);
    expect(possessedBadgeY).toBeCloseTo(2.3 * 1.15, 10);
    const src = readFileSync(resolve(process.cwd(), "src/render/worldView.ts"), "utf8");
