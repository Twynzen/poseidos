...
describe("constantes", () => {
  test("count 36, width 0.03, largo día 0.55 / noche 0.72, color 0xa8c4e0", () => {
    expect(RAIN_COUNT).toBe(36);
    expect(RAIN_STREAK_WIDTH).toBe(0.03);
    expect(RAIN_STREAK_LENGTH_DAY).toBe(0.55);
    expect(RAIN_STREAK_LENGTH_NIGHT).toBe(0.72);
    expect(RAIN_COLOR).toBe(0xa8c4e0);
  });

  test("opacity 0.22 + i×0.45; noche +0.20; active min 6; night cut 0.22; hide 0.02", () => {
    expect(RAIN_OPACITY_BASE).toBe(0.22);
    expect(RAIN_OPACITY_GAIN).toBe(0.45);
    expect(RAIN_OPACITY_NIGHT_ADD).toBe(0.2);
...
describe("rainNightMix / length / scaleY", () => {
  ...
  test("noche d=0 → mix 1, largo 0.72, scaleY 0.72/0.55", () => {
...
describe("rainStreakOpacity", () => {
  test("día: 0.22 + i×0.45; noche suma +0.20 × nightMix", () => {
    expect(rainStreakOpacity(0, 1)).toBeCloseTo(0.22, 10);
    expect(rainStreakOpacity(1, 1)).toBeCloseTo(0.22 + 0.45, 10);
    expect(rainStreakOpacity(1, 0)).toBeCloseTo(0.22 + 0.45 + 0.2, 10);
    expect(rainStreakOpacity(0.5, 0)).toBeCloseTo(0.22 + 0.5 * 0.45 + 0.2, 10);
    expect(rainStreakOpacity(1, 0.08)).toBeCloseTo(0.67 + 0.2 * 0.92, 10);
  });
});
