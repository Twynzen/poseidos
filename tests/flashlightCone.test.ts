...
describe("flashlightWedgeVertexColors", () => {
  test("ápice más brillante que el extremo lejano (tip → far fade); far R 0.184", () => {
    ...
    expect(c[1]).toBe(1);
    expect(c[2]).toBe(1);
    expect(c[3]).toBe(0.184);
    expect(c[3]).toBe(FLASHLIGHT_WEDGE_FAR_R);
    expect(c[4]).toBe(0.22);
    expect(c[5]).toBe(0.3);
    expect(c[6]).toBe(0.184);
    expect(c[6]).toBe(FLASHLIGHT_WEDGE_FAR_R);
    expect(c[7]).toBe(0.22);
    expect(c[8]).toBe(0.3);
  });
});
