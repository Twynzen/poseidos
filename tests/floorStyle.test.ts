...
describe("tintFromTile", () => {
  ...
  test("first outdoor swatch from OUTDOOR_GRASS_BASE 0x3d4f35 × 1.15", () => {
    ...
    expect(src).toContain("[0x53, 0x6c, 0x45]");
    expect(src).not.toContain("[0x48, 0x5e, 0x3c]");
  ...
  test("second outdoor swatch 0x455838 × 1.15", () => {
    const r0 = 0x4f;
    const g0 = 0x65;
    const b0 = 0x40;
    expect(Math.round(0x45 * 1.15)).toBe(r0);
    expect(Math.round(0x58 * 1.15)).toBe(g0);
    expect(Math.round(0x38 * 1.15)).toBe(b0);

    let found = false;
    for (let y = 0; y < 32 && !found; y++) {
      for (let x = 0; x < 32 && !found; x++) {
        const t = tileSeed01(x, y);
        if (Math.floor(t * 8) !== 1) continue;
        const j = tileSeed01(y, x);
        const dr = Math.floor((j - 0.5) * 12);
        const dg = Math.floor((t - 0.5) * 10);
        const db = Math.floor((j * t - 0.25) * 8);
  ...
  test("third outdoor swatch 0x34422e × 1.15", () => {
    const r0 = 0x3c;
    const g0 = 0x4c;
    const b0 = 0x35;
    expect(Math.round(0x34 * 1.15)).toBe(r0);
    expect(Math.round(0x42 * 1.15)).toBe(g0);
    expect(Math.round(0x2e * 1.15)).toBe(b0);

    let found = false;
    for (let y = 0; y < 32 && !found; y++) {
      for (let x = 0; x < 32 && !found; x++) {
        const t = tileSeed01(x, y);
        if (Math.floor(t * 8) !== 2) continue;
        const j = tileSeed01(y, x);
        const dr = Math.floor((j - 0.5) * 12);
        const dg = Math.floor((t - 0.5) * 10);
        const db = Math.floor((j * t - 0.25) * 8);
  ...
  test("fourth outdoor swatch 0x4a523a × 1.15", () => {
    const r0 = 0x55;
    const g0 = 0x5e;
    const b0 = 0x43;
    expect(Math.round(0x4a * 1.15)).toBe(r0);
    expect(Math.round(0x52 * 1.15)).toBe(g0);
    expect(Math.round(0x3a * 1.15)).toBe(b0);

    let found = false;
    for (let y = 0; y < 32 && !found; y++) {
      for (let x = 0; x < 32 && !found; x++) {
        const t = tileSeed01(x, y);
        if (Math.floor(t * 8) !== 3) continue;
        const j = tileSeed01(y, x);
        const dr = Math.floor((j - 0.5) * 12);
        const dg = Math.floor((t - 0.5) * 10);
        const db = Math.floor((j * t - 0.25) * 8);
  ...
  test("fifth outdoor swatch 0x3e4a36 × 1.15", () => {
    const r0 = 0x47;
    const g0 = 0x55;
    const b0 = 0x3e;
    expect(Math.round(0x3e * 1.15)).toBe(r0);
    expect(Math.round(0x4a * 1.15)).toBe(g0);
    expect(Math.round(0x36 * 1.15)).toBe(b0);

    let found = false;
    for (let y = 0; y < 32 && !found; y++) {
      for (let x = 0; x < 32 && !found; x++) {
        const t = tileSeed01(x, y);
        if (Math.floor(t * 8) !== 4) continue;
        const j = tileSeed01(y, x);
        const dr = Math.floor((j - 0.5) * 12);
        const dg = Math.floor((t - 0.5) * 10);
        const db = Math.floor((j * t - 0.25) * 8);
  ...
  test("sixth outdoor swatch 0x525a40 × 1.15", () => {
    const r0 = 0x5e;
    const g0 = 0x68;
    const b0 = 0x4a;
  ...
  test("eighth outdoor swatch 0x485e3c × 1.15", () => {
    const r0 = 0x53;
    const g0 = 0x6c;
    const b0 = 0x45;
    expect(Math.round(0x48 * 1.15)).toBe(r0);
    expect(Math.round(0x5e * 1.15)).toBe(g0);
    expect(Math.round(0x3c * 1.15)).toBe(b0);

    let found = false;
    for (let y = 0; y < 32 && !found; y++) {
      for (let x = 0; x < 32 && !found; x++) {
        const t = tileSeed01(x, y);
        if (Math.floor(t * 8) !== 7) continue;
        const j = tileSeed01(y, x);
        const dr = Math.floor((j - 0.5) * 12);
        const dg = Math.floor((t - 0.5) * 10);
        const db = Math.floor((j * t - 0.25) * 8);
...
describe("night wall albedo lift", () => {
  ...
  test("WALL_BASE_COLOR 0x1a1c22 × 1.15; worldView still uses it", () => {
    ...
    expect(BARRICADE_EDGE).toBe(0x9f7142);
    expect(GROUND_NIGHT_LIFT).toBe(1.6675);
    expect(AO_MAX_DARKEN).toBe(0.261);
    const viewSrc = readFileSync(
      resolve(process.cwd(), "src/render/worldView.ts"),
...
describe("night leftover prop albedo lift", () => {
  ...
  test("DOOR_OPEN 0xc4a35a × 1.15; worldView still uses it", () => {
    expect(DOOR_OPEN).toBe(0xe1bb68);
    const r = (DOOR_OPEN >> 16) & 0xff;
    const g = (DOOR_OPEN >> 8) & 0xff;
    const b = DOOR_OPEN & 0xff;
    expect(r).toBe(0xe1);
    expect(g).toBe(0xbb);
    expect(b).toBe(0x68);
    expect(Math.round((0xc4 * 115) / 100)).toBe(r);
    expect(Math.round((0xa3 * 115) / 100)).toBe(g);
    expect(Math.round((0x5a * 115) / 100)).toBe(b);
    expect(INDOOR_FLOOR_COLOR).toBe(0x303540);
    expect(OUTDOOR_GRASS_BASE).toBe(0x465b3d);
    expect(WALL_COLOR).toBe(0x685f53);
    expect(WALL_BASE_COLOR).toBe(0x1e2027);
...
describe("night leftover barricade albedo lift", () => {
  ...
  test("BARRICADE_EDGE 0x8a6239 × 1.15; worldView still uses it", () => {
    expect(BARRICADE_EDGE).toBe(0x9f7142);
    const r = (BARRICADE_EDGE >> 16) & 0xff;
    const g = (BARRICADE_EDGE >> 8) & 0xff;
    const b = BARRICADE_EDGE & 0xff;
    expect(r).toBe(0x9f);
    expect(g).toBe(0x71);
    expect(b).toBe(0x42);
    expect(Math.round((0x8a * 115) / 100)).toBe(r);
    expect(Math.round((0x62 * 115) / 100)).toBe(g);
    expect(Math.round((0x39 * 115) / 100)).toBe(b);
    expect(INDOOR_FLOOR_COLOR).toBe(0x303540);
    expect(OUTDOOR_GRASS_BASE).toBe(0x465b3d);
    expect(WALL_COLOR).toBe(0x685f53);
    expect(WALL_BASE_COLOR).toBe(0x1e2027);
  ...
  test("noche barricadas más claras; mismo multiply 1.6675", () => {
    ...
    expect(applyNightGroundLift(BARRICADE_EDGE, 0)).toBe(0xffbc6e);
