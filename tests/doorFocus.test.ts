...
describe("constantes", () => {
  test("reach 1.6; near 1.5525; far 1.12; pulse 0.05 / 6", () => {
  ...
  test("worldView aplica doorFocusMul al grupo de puerta existente", () => {
    ...
    expect(src).toContain("doorFocusMul(bestD, doorFocusElapsed)");
...
describe("doorFocusScale", () => {
  test("1.5525 en dist 0; 1.12 en reach; 1.0 fuera", () => {
    expect(doorFocusScale(0)).toBe(1.5525);
  ...
  test("lerp lineal dentro de reach", () => {
    // midpoint 0.8: 1.5525 + (1.12-1.5525)*0.5 = 1.33625
    expect(doorFocusScale(0.8)).toBeCloseTo(1.33625, 10);
    const t = 0.25;
  ...
  test("dist negativa se clampa a 0 → 1.5525", () => {
    expect(doorFocusScale(-0.4)).toBe(1.5525);
...
describe("doorFocusMul", () => {
  test("en reach: scale * pulse", () => {
    ...
    expect(doorFocusMul(1.6, elapsed)).toBeCloseTo(1.12 * 1.05, 10);
    expect(doorFocusMul(0.8, 0)).toBeCloseTo(1.33625, 10);
  });
