...
describe("possessedLook constants", () => {
  test("split knobs: fill levantado, map tint, crushed lock, emisivos", () => {
    ...
    expect(POSSESSED_BODY_EMISSIVE).toBe(0x261723);
    expect(POSSESSED_ACCENT).toBe(0x9f3062);
    expect(POSSESSED_ACCENT_COLOR).toBe(POSSESSED_ACCENT);
    expect(POSSESSED_ACCENT_EMISSIVE).toBe(0x55122e);
    expect(POSSESSED_FALLBACK_EMISSIVE).toBe(0x300917);
  ...
  test("fallback emissive 0x2a0814 × 1.15/canal → 0x300917; body/map-tint/emissive/accent/roughness/intensity/crushed iguales", () => {
    expect(POSSESSED_FALLBACK_EMISSIVE).toBe(0x300917);
    const r = (POSSESSED_FALLBACK_EMISSIVE >> 16) & 0xff;
    const g = (POSSESSED_FALLBACK_EMISSIVE >> 8) & 0xff;
    const b = POSSESSED_FALLBACK_EMISSIVE & 0xff;
    expect(r).toBe(0x30);
    expect(g).toBe(0x09);
    expect(b).toBe(0x17);
    expect(Math.round((0x2a * 115) / 100)).toBe(r);
    expect(Math.round((0x08 * 115) / 100)).toBe(g);
    expect(Math.round((0x14 * 115) / 100)).toBe(b);
    expect(POSSESSED_BODY_COLOR).toBe(0x432e40);
    expect(POSSESSED_MAP_TINT).toBe(0x9f7a96);
    expect(POSSESSED_BODY_EMISSIVE).toBe(0x261723);
    expect(POSSESSED_ACCENT).toBe(0x9f3062);
  ...
  test("accent emissive 0x4a1028 × 1.15/canal → 0x55122e; body/map-tint/accent/roughness/intensity/crushed iguales", () => {
    expect(POSSESSED_ACCENT_EMISSIVE).toBe(0x55122e);
    const r = (POSSESSED_ACCENT_EMISSIVE >> 16) & 0xff;
    const g = (POSSESSED_ACCENT_EMISSIVE >> 8) & 0xff;
    const b = POSSESSED_ACCENT_EMISSIVE & 0xff;
    expect(r).toBe(0x55);
    expect(g).toBe(0x12);
    expect(b).toBe(0x2e);
  ...
  test("accent 0x8a2a55 × 1.15/canal → 0x9f3062; body/map-tint/emissive/roughness/intensity/crushed iguales", () => {
    expect(POSSESSED_ACCENT).toBe(0x9f3062);
    const r = (POSSESSED_ACCENT >> 16) & 0xff;
    const g = (POSSESSED_ACCENT >> 8) & 0xff;
    const b = POSSESSED_ACCENT & 0xff;
    expect(r).toBe(0x9f);
    expect(g).toBe(0x30);
    expect(b).toBe(0x62);
    expect(Math.round((0x8a * 115) / 100)).toBe(r);
    expect(Math.round((0x2a * 115) / 100)).toBe(g);
    expect(Math.round((0x55 * 115) / 100)).toBe(b);
    expect(POSSESSED_ACCENT_COLOR).toBe(POSSESSED_ACCENT);
    expect(POSSESSED_BODY_COLOR).toBe(0x432e40);
    expect(POSSESSED_MAP_TINT).toBe(0x9f7a96);
    expect(POSSESSED_BODY_EMISSIVE).toBe(0x261723);
  ...
  test("body emissive 0x21141e × 1.15/canal → 0x261723; body/map-tint/accent/roughness/intensity/crushed iguales", () => {
    expect(POSSESSED_BODY_EMISSIVE).toBe(0x261723);
    const r = (POSSESSED_BODY_EMISSIVE >> 16) & 0xff;
    const g = (POSSESSED_BODY_EMISSIVE >> 8) & 0xff;
    const b = POSSESSED_BODY_EMISSIVE & 0xff;
    expect(r).toBe(0x26);
    expect(g).toBe(0x17);
    expect(b).toBe(0x23);
    expect(Math.round((0x21 * 115) / 100)).toBe(r);
    expect(Math.round((0x14 * 115) / 100)).toBe(g);
    expect(Math.round((0x1e * 115) / 100)).toBe(b);
    expect(POSSESSED_BODY_COLOR).toBe(0x432e40);
    expect(POSSESSED_MAP_TINT).toBe(0x9f7a96);
    expect(POSSESSED_ACCENT).toBe(0x9f3062);
    expect(POSSESSED_ACCENT_COLOR).toBe(POSSESSED_ACCENT);
