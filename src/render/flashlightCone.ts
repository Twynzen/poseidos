...
/** Idle fill intensity. Ctor torchLight.intensity 0 = fresco. Mid-life leftover ≠ 0. */
export const FLASHLIGHT_FILL_INTENSITY_SPAWN = 0;

/**
 * Intensity que lee syncTorchLight (look fresco o vivo).
 * leftover mid-life on ≠ fresco (idle 0).
 */
export function flashlightFillIntensityFromLook(intensity: number): number {
  return intensity;
}

/**
 * R / softReset: intensity fresco (idle 0).
 * WorldView nace torchLight.intensity AfterRestart; leftover mid-life on no filtra.
 * syncTorchLight lee flashlightFillIntensityFromLook.
 * F9 / enterGameOver / freeze death no assign.
 */
export function flashlightFillIntensityAfterRestart(): number {
...
/** Idle spot distance. Ctor torchSpot.distance LENGTH+2.4 leftover vs idle LENGTH+EXTRA 8.227675. Mid-life leftover ≠ 8.227675. */
export const FLASHLIGHT_SPOT_DISTANCE_SPAWN = 8.227675;

/**
 * Distance que lee syncTorchLight (look fresco o vivo).
 * leftover mid-life / ctor LENGTH+2.4 ≠ fresco (idle LENGTH+EXTRA 8.227675).
 */
export function flashlightSpotDistanceFromLook(distance: number): number {
  return distance;
}

/**
 * R / softReset: distance fresco (idle LENGTH+EXTRA 8.227675 / intensity-0).
 * WorldView nace torchSpot.distance AfterRestart; leftover ctor LENGTH+2.4 / mid-life no filtra.
 * syncTorchLight lee flashlightSpotDistanceFromLook.
 * F9 / enterGameOver / freeze death no assign.
 */
export function flashlightSpotDistanceAfterRestart(): number {
...
/** Idle spot decay. Ctor torchSpot.decay FLASHLIGHT_SPOT_DECAY = fresco. Mid-life leftover ≠ fresco. */
export const FLASHLIGHT_SPOT_DECAY_SPAWN = 1.74;

/**
 * Decay que leería syncTorchLight (look fresco o vivo).
 * leftover mid-life ≠ fresco (idle FLASHLIGHT_SPOT_DECAY).
 * syncTorchLight no escribe decay (ctor constant).
 */
export function flashlightSpotDecayFromLook(decay: number): number {
  return decay;
}

/**
 * R / softReset: decay fresco (idle FLASHLIGHT_SPOT_DECAY).
 * WorldView nace torchSpot.decay AfterRestart; leftover mid-life no filtra.
 * syncTorchLight no escribe decay (ctor constant).
 * F9 / enterGameOver / freeze death no assign.
 */
export function flashlightSpotDecayAfterRestart(): number {
  return flashlightSpotDecayFromLook(FLASHLIGHT_SPOT_DECAY_SPAWN);
}
...
/** Idle flashlight cone mesh color. Ctor coneMat.color FLASHLIGHT_WEDGE_COLOR 0xefffff = fresco. Mid-life leftover ≠ fresco. */
export const FLASHLIGHT_WEDGE_COLOR_SPAWN = 0xefffff;

/**
 * Color que leería syncTorchLight (look fresco o vivo).
 * leftover mid-life ≠ fresco (idle FLASHLIGHT_WEDGE_COLOR 0xefffff).
 * syncTorchLight no escribe color (ctor constant).
 */
export function flashlightConeColorFromLook(color: number): number {
  return color;
}

/**
 * R / softReset: color fresco (idle FLASHLIGHT_WEDGE_COLOR 0xefffff).
 * WorldView nace coneMat.color AfterRestart; leftover mid-life no filtra.
 * syncTorchLight no escribe color (ctor constant).
 * F9 / enterGameOver / freeze death no assign.
 */
