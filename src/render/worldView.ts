...
/** DepthWrite del player-body mesh. Ctor playerBodyMat.depthWrite: true = fresco. Mid-life leftover ≠ fresco. */
export const PLAYER_BODY_DEPTH_WRITE = true;

/** Idle player-body mesh depthWrite. Ctor playerBodyMat.depthWrite: true = fresco. Mid-life leftover ≠ fresco. */
export const PLAYER_BODY_DEPTH_WRITE_SPAWN = true;
 ...
 */
export function playerBodyDepthWriteFromLook(depthWrite: boolean): boolean {
  return depthWrite;
}

/**
 * R / softReset: depthWrite fresco (idle true).
 * WorldView nace playerBodyMat.depthWrite AfterRestart; leftover mid-life no filtra.
 * attach/tick no escriben depthWrite (ctor constant).
 * F9 / enterGameOver / freeze death no assign.
 */
export function playerBodyDepthWriteAfterRestart(): boolean {
  return playerBodyDepthWriteFromLook(PLAYER_BODY_DEPTH_WRITE_SPAWN);
}

/** Side del player-body mesh. Ctor playerBodyMat.side THREE.FrontSide (0) = fresco. Mid-life leftover ≠ fresco. */
export const PLAYER_BODY_SIDE = 0;

/** Idle player-body mesh side. Ctor playerBodyMat.side THREE.FrontSide (0) = fresco. Mid-life leftover ≠ fresco. */
export const PLAYER_BODY_SIDE_SPAWN = 0;
 ...
 */
export function playerBodySideFromLook(side: number): number {
  return side;
}

/**
 * R / softReset: side fresco (idle THREE.FrontSide / 0).
 * WorldView nace playerBodyMat.side AfterRestart; leftover mid-life no filtra.
 * attach/tick no escriben side (ctor constant).
 * F9 / enterGameOver / freeze death no assign.
 */
export function playerBodySideAfterRestart(): number {
  return playerBodySideFromLook(PLAYER_BODY_SIDE_SPAWN);
}
...
/** DepthWrite del player-head mesh. Ctor playerHeadMat.depthWrite: true = fresco. Mid-life leftover ≠ fresco. */
export const PLAYER_HEAD_DEPTH_WRITE = true;

/** Idle player-head mesh depthWrite. Ctor playerHeadMat.depthWrite: true = fresco. Mid-life leftover ≠ fresco. */
export const PLAYER_HEAD_DEPTH_WRITE_SPAWN = true;
 ...
 * attach/tick no escriben depthWrite (ctor constant).
 */
export function playerHeadDepthWriteFromLook(depthWrite: boolean): boolean {
  return depthWrite;
}

/**
 * R / softReset: depthWrite fresco (idle true).
 * WorldView nace playerHeadMat.depthWrite AfterRestart; leftover mid-life no filtra.
 * attach/tick no escriben depthWrite (ctor constant).
 * F9 / enterGameOver / freeze death no assign.
 */
export function playerHeadDepthWriteAfterRestart(): boolean {
...
/** Side del possessed mesh. Ctor possessedMat.side THREE.FrontSide (0) = fresco. Mid-life leftover ≠ fresco. */
export const POSSESSED_SIDE = 0;

/** Idle possessed mesh side. Ctor possessedMat.side THREE.FrontSide (0) = fresco. Mid-life leftover ≠ fresco. */
export const POSSESSED_SIDE_SPAWN = 0;
 ...
 * attach/tick no escriben side (ctor constant).
 */
export function possessedSideFromLook(side: number): number {
  return side;
}

/**
 * R / softReset: side fresco (idle THREE.FrontSide / 0).
 * WorldView nace possessedMat.side AfterRestart; leftover mid-life no filtra.
 * attach/tick no escriben side (ctor constant).
 * F9 / enterGameOver / freeze death no assign.
 */
export function possessedSideAfterRestart(): number {
  return possessedSideFromLook(POSSESSED_SIDE_SPAWN);
}
...
/** Side del bed mesh. Ctor bedMat.side THREE.FrontSide (0) = fresco. Mid-life leftover ≠ fresco. */
export const BED_SIDE = 0;

/** Idle bed mesh side. Ctor bedMat.side THREE.FrontSide (0) = fresco. Mid-life leftover ≠ fresco. */
export const BED_SIDE_SPAWN = 0;
 ...
 * attach/tick no escriben side (ctor constant).
 */
export function bedSideFromLook(side: number): number {
  return side;
}

/**
 * R / softReset: side fresco (idle THREE.FrontSide / 0).
 * WorldView nace bedMat.side AfterRestart; leftover mid-life no filtra.
 * attach/tick no escriben side (ctor constant).
 * F9 / enterGameOver / freeze death no assign.
 */
export function bedSideAfterRestart(): number {
  return bedSideFromLook(BED_SIDE_SPAWN);
}
...
/** Side del door-closed mesh. Ctor doorClosedMat.side THREE.FrontSide (0) = fresco. Mid-life leftover ≠ fresco. */
export const DOOR_CLOSED_SIDE = 0;

/** Idle door-closed mesh side. Ctor doorClosedMat.side THREE.FrontSide (0) = fresco. Mid-life leftover ≠ fresco. */
export const DOOR_CLOSED_SIDE_SPAWN = 0;
 ...
 * attach/tick no escriben side (ctor constant).
 */
export function doorClosedSideFromLook(side: number): number {
  return side;
}

/**
 * R / softReset: side fresco (idle THREE.FrontSide / 0).
 * WorldView nace doorClosedMat.side AfterRestart; leftover mid-life no filtra.
 * attach/tick no escriben side (ctor constant).
 * F9 / enterGameOver / freeze death no assign.
 */
export function doorClosedSideAfterRestart(): number {
  return doorClosedSideFromLook(DOOR_CLOSED_SIDE_SPAWN);
}
...
/** DepthWrite del door-open mesh. Ctor doorOpenMat.depthWrite: true = fresco. Mid-life leftover ≠ fresco. */
export const DOOR_OPEN_DEPTH_WRITE = true;

/** Idle door-open mesh depthWrite. Ctor doorOpenMat.depthWrite: true = fresco. Mid-life leftover ≠ fresco. */
export const DOOR_OPEN_DEPTH_WRITE_SPAWN = true;
 ...
 * attach/tick no escriben depthWrite (ctor constant).
 */
export function doorOpenDepthWriteFromLook(depthWrite: boolean): boolean {
  return depthWrite;
}

/**
 * R / softReset: depthWrite fresco (idle true).
 * WorldView nace doorOpenMat.depthWrite AfterRestart; leftover mid-life no filtra.
 * attach/tick no escriben depthWrite (ctor constant).
 * F9 / enterGameOver / freeze death no assign.
 */
export function doorOpenDepthWriteAfterRestart(): boolean {
...
/** DepthWrite del floor mesh. Ctor floorMat.depthWrite: true = fresco. Mid-life leftover ≠ fresco. */
export const FLOOR_DEPTH_WRITE = true;

/** Idle floor mesh depthWrite. Ctor floorMat.depthWrite: true = fresco. Mid-life leftover ≠ fresco. */
export const FLOOR_DEPTH_WRITE_SPAWN = true;
 ...
 * attach/tick no escriben depthWrite (ctor constant).
 */
export function floorDepthWriteFromLook(depthWrite: boolean): boolean {
  return depthWrite;
}

/**
 * R / softReset: depthWrite fresco (idle true).
 * WorldView nace floorMat.depthWrite AfterRestart; leftover mid-life no filtra.
 * attach/tick no escriben depthWrite (ctor constant).
 * F9 / enterGameOver / freeze death no assign.
 */
export function floorDepthWriteAfterRestart(): boolean {
...
/** Side del wall mesh. Ctor wallMat.side THREE.FrontSide (0) = fresco. Mid-life leftover ≠ fresco. */
export const WALL_SIDE = 0;

/** Idle wall mesh side. Ctor wallMat.side THREE.FrontSide (0) = fresco. Mid-life leftover ≠ fresco. */
export const WALL_SIDE_SPAWN = 0;
 ...
 * attach/tick no escriben side (ctor constant).
 */
export function wallSideFromLook(side: number): number {
  return side;
}

/**
 * R / softReset: side fresco (idle THREE.FrontSide / 0).
 * WorldView nace wallMat.side AfterRestart; leftover mid-life no filtra.
 * attach/tick no escriben side (ctor constant).
 * F9 / enterGameOver / freeze death no assign.
 */
export function wallSideAfterRestart(): number {
  return wallSideFromLook(WALL_SIDE_SPAWN);
}
...
/** DepthWrite del wall-base mesh. Ctor wallBaseMat.depthWrite: true = fresco. Mid-life leftover ≠ fresco. */
export const WALL_BASE_DEPTH_WRITE = true;

/** Idle wall-base mesh depthWrite. Ctor wallBaseMat.depthWrite: true = fresco. Mid-life leftover ≠ fresco. */
export const WALL_BASE_DEPTH_WRITE_SPAWN = true;
 ...
 * attach/tick no escriben depthWrite (ctor constant).
 */
export function wallBaseDepthWriteFromLook(depthWrite: boolean): boolean {
  return depthWrite;
}

/**
 * R / softReset: depthWrite fresco (idle true).
 * WorldView nace wallBaseMat.depthWrite AfterRestart; leftover mid-life no filtra.
 * attach/tick no escriben depthWrite (ctor constant).
 * F9 / enterGameOver / freeze death no assign.
 */
export function wallBaseDepthWriteAfterRestart(): boolean {
...
/** Transparent del barricade mesh. Ctor barricadeMat.transparent: false = fresco. Mid-life leftover ≠ fresco. */
export const BARRICADE_TRANSPARENT = false;

/** Idle barricade mesh transparent. Ctor barricadeMat.transparent: false = fresco. Mid-life leftover ≠ fresco. */
export const BARRICADE_TRANSPARENT_SPAWN = false;
 ...
 * attach/tick no escriben transparent (ctor constant).
 */
export function barricadeTransparentFromLook(transparent: boolean): boolean {
  return transparent;
}

/**
 * R / softReset: transparent fresco (idle false).
 * WorldView nace barricadeMat.transparent AfterRestart; leftover mid-life no filtra.
 * attach/tick no escriben transparent (ctor constant).
 * F9 / enterGameOver / freeze death no assign.
 */
export function barricadeTransparentAfterRestart(): boolean {
  return barricadeTransparentFromLook(BARRICADE_TRANSPARENT_SPAWN);
}

/** DepthWrite del barricade mesh. Ctor barricadeMat.depthWrite: true = fresco. Mid-life leftover ≠ fresco. */
export const BARRICADE_DEPTH_WRITE = true;

/** Idle barricade mesh depthWrite. Ctor barricadeMat.depthWrite: true = fresco. Mid-life leftover ≠ fresco. */
export const BARRICADE_DEPTH_WRITE_SPAWN = true;
 ...
 * attach/tick no escriben depthWrite (ctor constant).
 */
export function barricadeDepthWriteFromLook(depthWrite: boolean): boolean {
  return depthWrite;
}

/**
 * R / softReset: depthWrite fresco (idle true).
 * WorldView nace barricadeMat.depthWrite AfterRestart; leftover mid-life no filtra.
 * attach/tick no escriben depthWrite (ctor constant).
 * F9 / enterGameOver / freeze death no assign.
 */
export function barricadeDepthWriteAfterRestart(): boolean {
  return barricadeDepthWriteFromLook(BARRICADE_DEPTH_WRITE_SPAWN);
}

/** Side del barricade mesh. Ctor barricadeMat.side THREE.FrontSide (0) = fresco. Mid-life leftover ≠ fresco. */
export const BARRICADE_SIDE = 0;

/** Idle barricade mesh side. Ctor barricadeMat.side THREE.FrontSide (0) = fresco. Mid-life leftover ≠ fresco. */
export const BARRICADE_SIDE_SPAWN = 0;
 ...
 * attach/tick no escriben side (ctor constant).
 */
export function barricadeSideFromLook(side: number): number {
  return side;
}

/**
 * R / softReset: side fresco (idle THREE.FrontSide / 0).
 * WorldView nace barricadeMat.side AfterRestart; leftover mid-life no filtra.
 * attach/tick no escriben side (ctor constant).
 * F9 / enterGameOver / freeze death no assign.
 */
export function barricadeSideAfterRestart(): number {
  return barricadeSideFromLook(BARRICADE_SIDE_SPAWN);
}
...
/** Transparent del barricade-edge mesh. Ctor barricadeEdgeMat.transparent: false = fresco. Mid-life leftover ≠ fresco. */
export const BARRICADE_EDGE_TRANSPARENT = false;

/** Idle barricade-edge mesh transparent. Ctor barricadeEdgeMat.transparent: false = fresco. Mid-life leftover ≠ fresco. */
export const BARRICADE_EDGE_TRANSPARENT_SPAWN = false;
 ...
 * attach/tick no escriben transparent (ctor constant).
 */
export function barricadeEdgeTransparentFromLook(transparent: boolean): boolean {
  return transparent;
}

/**
 * R / softReset: transparent fresco (idle false).
 * WorldView nace barricadeEdgeMat.transparent AfterRestart; leftover mid-life no filtra.
 * attach/tick no escriben transparent (ctor constant).
 * F9 / enterGameOver / freeze death no assign.
 */
export function barricadeEdgeTransparentAfterRestart(): boolean {
  return barricadeEdgeTransparentFromLook(BARRICADE_EDGE_TRANSPARENT_SPAWN);
}

/** DepthWrite del barricade-edge mesh. Ctor barricadeEdgeMat.depthWrite: true = fresco. Mid-life leftover ≠ fresco. */
export const BARRICADE_EDGE_DEPTH_WRITE = true;

/** Idle barricade-edge mesh depthWrite. Ctor barricadeEdgeMat.depthWrite: true = fresco. Mid-life leftover ≠ fresco. */
export const BARRICADE_EDGE_DEPTH_WRITE_SPAWN = true;
 ...
 * attach/tick no escriben depthWrite (ctor constant).
 */
export function barricadeEdgeDepthWriteFromLook(depthWrite: boolean): boolean {
  return depthWrite;
}

/**
 * R / softReset: depthWrite fresco (idle true).
 * WorldView nace barricadeEdgeMat.depthWrite AfterRestart; leftover mid-life no filtra.
 * attach/tick no escriben depthWrite (ctor constant).
 * F9 / enterGameOver / freeze death no assign.
 */
export function barricadeEdgeDepthWriteAfterRestart(): boolean {
...
export function createWorldView(
  ...
  const bedMat = new THREE.MeshStandardMaterial({
    ...
    // R / dispose: side fresco (idle); leftover mid-life side de la vida anterior no filtra.
    side: bedSideAfterRestart() as THREE.Side,
  });
  const doorClosedMat = new THREE.MeshStandardMaterial({
    ...
    // R / dispose: side fresco (idle); leftover mid-life side de la vida anterior no filtra.
    side: doorClosedSideAfterRestart() as THREE.Side,
  });
  ...
  const wallMat = new THREE.MeshStandardMaterial({
    ...
    // R / dispose: side fresco (idle); leftover mid-life side de la vida anterior no filtra.
    side: wallSideAfterRestart() as THREE.Side,
  });
  ...
  const barricadeMat = new THREE.MeshStandardMaterial({
    ...
    // R / dispose: transparent fresco (idle); leftover mid-life transparent de la vida anterior no filtra.
    transparent: barricadeTransparentAfterRestart(),
    // R / dispose: depthWrite fresco (idle); leftover mid-life depthWrite de la vida anterior no filtra.
    depthWrite: barricadeDepthWriteAfterRestart(),
    // R / dispose: side fresco (idle); leftover mid-life side de la vida anterior no filtra.
    side: barricadeSideAfterRestart() as THREE.Side,
  });
  ...
  const playerBodyMat = new THREE.MeshStandardMaterial({
    ...
    // R / dispose: depthWrite fresco (idle); leftover mid-life depthWrite de la vida anterior no filtra.
    depthWrite: playerBodyDepthWriteAfterRestart(),
    // R / dispose: side fresco (idle); leftover mid-life side de la vida anterior no filtra.
    side: playerBodySideAfterRestart() as THREE.Side,
  });
  ...
  function makeLootNameplateSprite(
    ...
    const mat = new THREE.SpriteMaterial({
      ...
      // R / dispose: side fresco (idle); leftover mid-life side de la vida anterior no filtra.
      side: lootNameplateSideAfterRestart() as THREE.Side,
      // R / dispose: sizeAttenuation fresco (idle); leftover mid-life sizeAttenuation de la vida anterior no filtra.
      sizeAttenuation: lootNameplateSizeAttenuationAfterRestart(),
      // R / dispose: fog fresco (idle); leftover mid-life fog de la vida anterior no filtra.
      ...
      // R / dispose: colorWrite fresco (idle); leftover mid-life colorWrite de la vida anterior no filtra.
      colorWrite: lootNameplateColorWriteAfterRestart(),
      // R / dispose: dithering fresco (idle); leftover mid-life dithering de la vida anterior no filtra.
      dithering: lootNameplateDitheringAfterRestart(),
      // R / dispose: premultipliedAlpha fresco (idle); leftover mid-life premultipliedAlpha de la vida anterior no filtra.
      premultipliedAlpha: lootNameplatePremultipliedAlphaAfterRestart(),
      // R / dispose: toneMapped fresco (idle); leftover mid-life toneMapped de la vida anterior no filtra.
      ...
      // R / dispose: alphaHash fresco (idle); leftover mid-life alphaHash de la vida anterior no filtra.
      ...
      // R / dispose: blendDst fresco (idle); leftover mid-life blendDst de la vida anterior no filtra.
      blendDst: lootNameplateBlendDstAfterRestart() as THREE.BlendingDstFactor,
      // R / dispose: blendEquation fresco (idle); leftover mid-life blendEquation de la vida anterior no filtra.
      blendEquation: lootNameplateBlendEquationAfterRestart() as THREE.BlendingEquation,
      // R / dispose: blendSrcAlpha fresco (idle); leftover mid-life blendSrcAlpha de la vida anterior no filtra.
      blendSrcAlpha: lootNameplateBlendSrcAlphaAfterRestart() as THREE.BlendingSrcFactor | null,
      // R / dispose: blendDstAlpha fresco (idle); leftover mid-life blendDstAlpha de la vida anterior no filtra.
      blendDstAlpha: lootNameplateBlendDstAlphaAfterRestart() as THREE.BlendingDstFactor | null,
      // R / dispose: blendEquationAlpha fresco (idle); leftover mid-life blendEquationAlpha de la vida anterior no filtra.
      blendEquationAlpha: lootNameplateBlendEquationAlphaAfterRestart() as THREE.BlendingEquation | null,
      // R / dispose: blendColor fresco (idle); leftover mid-life blendColor de la vida anterior no filtra.
      blendColor: lootNameplateBlendColorAfterRestart(),
      // R / dispose: blendAlpha fresco (idle); leftover mid-life blendAlpha de la vida anterior no filtra.
      ...
      // R / dispose: stencilFunc fresco (idle); leftover mid-life stencilFunc de la vida anterior no filtra.
      stencilFunc: lootNameplateStencilFuncAfterRestart() as THREE.StencilFunc,
      // R / dispose: stencilRef fresco (idle); leftover mid-life stencilRef de la vida anterior no filtra.
      stencilRef: lootNameplateStencilRefAfterRestart(),
      // R / dispose: stencilWriteMask fresco (idle); leftover mid-life stencilWriteMask de la vida anterior no filtra.
      stencilWriteMask: lootNameplateStencilWriteMaskAfterRestart(),
      // R / dispose: stencilFuncMask fresco (idle); leftover mid-life stencilFuncMask de la vida anterior no filtra.
      ...
      // R / dispose: stencilZPass fresco (idle); leftover mid-life stencilZPass de la vida anterior no filtra.
      ...
      // R / dispose: clipShadows fresco (idle); leftover mid-life clipShadows de la vida anterior no filtra.
      ...
      // R / dispose: shadowSide fresco (idle); leftover mid-life shadowSide de la vida anterior no filtra.
      shadowSide: lootNameplateShadowSideAfterRestart(),
      // R / dispose: alphaMap fresco (idle); leftover mid-life alphaMap de la vida anterior no filtra.
      alphaMap: lootNameplateAlphaMapAfterRestart(),
      // R / dispose: Material.visible fresco (idle); leftover mid-life Material.visible de la vida anterior no filtra.
    ...
    // R / dispose: Object3D.userData fresco (idle); leftover mid-life Object3D.userData de la vida anterior no filtra.
    sprite.userData = lootNameplateObjectUserDataAfterRestart();
    // R / dispose: matrixWorldAutoUpdate fresco (idle); leftover mid-life matrixWorldAutoUpdate de la vida anterior no filtra.
    sprite.matrixWorldAutoUpdate = lootNameplateMatrixWorldAutoUpdateAfterRestart();
    // R / dispose: up fresco (idle); leftover mid-life up de la vida anterior no filtra.
    ...
    // R / dispose: customDistanceMaterial fresco (idle); leftover mid-life customDistanceMaterial de la vida anterior no filtra.
  ...
  const muzzleMat = new THREE.MeshBasicMaterial({
    ...
    // R / dispose: side fresco (idle); leftover mid-life side de la vida anterior no filtra.
    side: muzzleSideAfterRestart() as THREE.Side,
  });
  ...
  const possessedMat = new THREE.MeshStandardMaterial({
    ...
    // R / dispose: side fresco (idle); leftover mid-life side de la vida anterior no filtra.
    side: possessedSideAfterRestart() as THREE.Side,
  });
  ...
  const grassMat = new THREE.MeshStandardMaterial({
    ...
    // R / dispose: side fresco (idle); leftover mid-life side de la vida anterior no filtra.
    side: grassSideAfterRestart() as THREE.Side,
  });
