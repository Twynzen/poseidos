...
export class Game {
  ...
  private softReset(): void {
    ...
    // R: WorldView nuevo = spawn + bed/door/nameplate/loot + grass + rain + camera + muzzle + impact + tracers + melee + loco + hit lean + camera shake + noise rings + facing chevron + flashlight cone + flashlight cone visible + torch fill + torch spot + torch spot target + warm light fresco; leftover origin 0,0 / target mid-life / visible leftover / opacity 1 / mid-swing / mid-stride / mid-recoil / mid-shake / mid-life ring / mid-life yaw no filtra.
    this.view.dispose();
    this.view = createWorldView(this.map, this.containers);
