...
export class Game {
  ...
  private softReset(): void {
    ...
    // R: WorldView nuevo = spawn + bed/door/nameplate/loot + grass + rain + camera + muzzle + impact + tracers + melee + loco + hit lean + camera shake + noise rings + facing chevron + flashlight cone + torch fill + torch spot fresco; leftover origin 0,0 / opacity 1 / mid-swing / mid-stride / mid-recoil / mid-shake / mid-life ring / mid-life yaw no filtra.
    this.view.dispose();
    this.view = createWorldView(this.map, this.containers);
