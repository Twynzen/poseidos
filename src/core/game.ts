...
export class Game {
  ...
  private softReset(): void {
    ...
    // R: WorldView nuevo = spawn + bed/door/nameplate/loot + loot nameplate sprite transparent + loot nameplate sprite depthWrite + loot nameplate sprite color + grass/roughness + rain + rain mesh color + rain mesh transparent + fog mesh color + fog mesh opacity + fog mesh Y + fog mesh rot + fog mesh rot Y + fog mesh rot Z + fog mesh scale + fog mesh scale Y + fog mesh scale Z + fog mesh transparent/depthWrite + camera + muzzle + muzzle light decay + muzzle light color + muzzle light distance + muzzle flash mesh color + impact + impact spark light decay + impact spark light color + impact spark light distance + impact spark mesh color + tracers + tracer flash decay + tracer flash color + tracer flash distance + tracer mesh color + melee + loco + hit lean + camera shake + noise rings + facing chevron + facing chevron mesh color + facing chevron mesh opacity + facing chevron mesh transparent + flashlight cone + flashlight cone visible + flashlight cone opacity + flashlight fill distance + flashlight fill intensity + flashlight fill decay + flashlight spot distance + flashlight spot intensity + flashlight spot angle + flashlight spot penumbra + flashlight spot decay + torch fill + torch spot + torch spot target + warm light + warm light distance/intensity/Y/color/decay fresco; leftover origin 0,0 / target mid-life / visible leftover / ctor BASE opacity / ctor 10 leftover / ctor LENGTH+2.4 leftover / ctor 7.5/1.6/0xffca81 leftover / mid-life intensity / opacity 1 / mid-swing / mid-stride / mid-recoil / mid-shake / mid-life ring / mid-life yaw no filtra.
    this.view.dispose();
    this.view = createWorldView(this.map, this.containers);
